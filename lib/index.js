...
class ErrsoleSequelize {
  ...
  async syncModels () {
    try {
      ...
      console.log('Sucessfully connected to Sequelize');
      await this.sequelize.sync({ alter: true });
    } catch (error) {
      console.error('Failed to connect to Sequelize:', error);
    }
  }
  ...
  async getUserProfile (email) {
    ...
    try {
      const user = await this.errsoleUsers.findOne({ where: { email }, attributes: { exclude: ['password'] } });
