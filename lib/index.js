 ...
 */
  ...
  async defineModels () {
    this.errsoleLogs = this.sequelize.define('errsoleLog', {
      ...
      hostname: { type: DataTypes.STRING(54) },
      pid: { type: DataTypes.BIGINT }
    }, {
      timestamps: false, // Disabling the createdAt and updatedAt columns while creating the models
      indexes: [
        { type: 'FULLTEXT', name: 'text', fields: ['message'] },
        { name: 'idx_source_level_id', fields: ['source', 'level', 'id'] },
        { name: 'idx_source_level_timestamp', fields: ['source', 'level', 'timestamp'] },
        { name: 'idx_hostname_pid_id', fields: ['hostname', 'pid', 'id'] }
      ]
    });
   ...
   */
  async getConfig (key) {
    const configEntry = await this.errsoleConfigs.findOne({ where: { key } });
  ...
  async setConfig (key, value) {
    const result = await this.errsoleConfigs.upsert({ key, value });
    if (result) {
