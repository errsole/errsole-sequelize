...
class ErrsoleSequelize extends EventEmitter {
  ...
  async syncModels () {
    ...
    try {
      await this.sequelize.query('CREATE INDEX errsole_logs_timestamp ON errsole_logs(timestamp);');
    } catch {}
  ...
  async searchLogs (searchTerms, filters = {}) {
    ...
    if (filters.lte_timestamp && !filters.gte_timestamp) {
      ...
      filters.gte_timestamp = new Date(filters.lte_timestamp.getTime() - 24 * 60 * 60 * 1000);
    }
    if (filters.gte_timestamp && !filters.lte_timestamp) {
      filters.gte_timestamp = new Date(filters.gte_timestamp);
      filters.lte_timestamp = new Date(filters.gte_timestamp.getTime() + 24 * 60 * 60 * 1000);
    }
