...
class ErrsoleSequelize extends EventEmitter {
  ...
  async postLogs (logEntries) {
    ...
    try {
      ...
      const sql = 'INSERT INTO errsole_logs (hostname, pid, source, timestamp, level, message, meta) VALUES ?';
  ...
  async deleteExpiredLogs () {
    ...
    try {
      ...
      const configResult = await this.getConfig('logsTTL');
      if (configResult.item) {
        const parsedTTL = parseInt(configResult.item.value, 10) * 24 * 60 * 60 * 1000;
