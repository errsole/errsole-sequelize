...
class ErrsoleSequelize extends EventEmitter {
  ...
  /**
  * Retrieves the metadata associated with a log entry.
  *
  * @async
  * @function getMeta
  * @param {number} logId - The unique ID of the log entry.
  * @returns {Promise<{item: meta, id}>}  - A Promise that resolves with an object containing the log ID and its associated metadata.
  * @throws {Error} - Throws an error if the log ID is not provided or if the metadata retrieval fails.
  */
  async getMeta (logId) {
    ...
    if (!result) throw new Error('Meta is not found with this log ID.');

    return { item: { meta: result.meta, id: result.id } };
  }
}
