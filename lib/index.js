...
class ErrsoleSequelize extends EventEmitter {
  ...
  async getLogs (filters = {}) {
    ...
    if (this.errsoleLogs.sequelize.getDialect() === 'mysql') {
      await this.errsoleLogs.sequelize.query('SET SESSION sort_buffer_size = 8388608;');
    }

    // Setting work_mem for PostgreSQL only for transaction blocks
    if (this.errsoleLogs.sequelize.getDialect() === 'postgres') {
      this.errsoleLogs.sequelize.query("SET LOCAL work_mem = '8MB';");
    }

    if (this.errsoleLogs.sequelize.getDialect() === 'sqlite') {
