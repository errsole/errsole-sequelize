...
const saltRounds = 10;
const defaultTTL = 2592000000; // 30 days in milliseconds
...
class ErrsoleSequelize extends EventEmitter {
  ...
  */
  ...
  /**
   * Ensures that the Time To Live (TTL) configuration for logs is set and updates the TTL for the logs collection.
   *
   * @async
   * @function ensureLogsTTL
   * @returns {Promise<void>} - A Promise that resolves when the TTL settings are successfully ensured or updated.
   * @throws {Error} - Throws an error if there are issues retrieving or setting the configuration, or updating the logs collection.
   */
  async ensureLogsTTL () {
    try {
      const config = await this.errsoleConfig.findOne({ where: { key: 'logsTTL' }, raw: true });
      if (!config) {
        // If no TTL is set, set a default TTL
        await this.errsoleConfig.create({ key: 'logsTTL', value: defaultTTL.toString() });
      }
      await this.deleteExpiredLogs();
    } catch (err) {
      console.error('Error ensuring logs TTL:', err);
    }
  }
  ...
  async deleteExpiredLogs () {
    ...
    const config = await this.errsoleConfig.findOne({ where: { key: 'logsTTL' }, raw: true });
    if (!config) {
      console.error('Problem in creating cron job to delete older logs');
      return false;
    }
    const logsTTL = parseInt(config.value, 10);
    // Utility function to delay execution
    function delay (ms) {
      return new Promise(resolve => setTimeout(resolve, ms));
    }
    cron.schedule('0 * * * *', async function deleteOldLogs () {
      while (true) {
        const deleted = await self.errsoleLogs.destroy({
          where: {
            timestamp: {
              [Sequelize.Op.lt]: new Date(new Date().getTime() - logsTTL)
            }
          },
          limit: batchSize
        });

        if (!deleted) break; // exit loop if no records are deleted
