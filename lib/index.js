 ...
 */
  ...
  async getLogs (filters = {}) {
    ...
    if (filters.sources) {
      query.source = { [Sequelize.Op.in]: filters.sources };
    }
    if (filters.levels) {
      query.level = { [Sequelize.Op.in]: filters.levels };
    }

    if (filters.gt_id) {
      query.id = { [Sequelize.Op.gt]: filters.gt_id };
    } else if (filters.lt_id) {
      query.id = { [Sequelize.Op.lt]: filters.lt_id };
    }

    if (filters.gte_timestamp || filters.lte_timestamp) {
      query.timestamp = {};
      if (filters.gte_timestamp) {
        query.timestamp[Sequelize.Op.gte] = new Date(filters.gte_timestamp);
      }
      if (filters.lte_timestamp) {
        query.timestamp[Sequelize.Op.lte] = new Date(filters.lte_timestamp);
      }
    }
    ...
    const documents = await this.errsoleLogs.findAll({
      where: query,
      order: sortOrder,
      limit: filters.limit
    });

    // Reverse the order of documents if required
    if (shouldReverse) {
      documents.reverse();
    }

    // Format documents to match the expected output
  ...
  async searchLogs (searchTerms, filters = {}) {
    ...
    const words = searchTerms.split(/\s+/); // Splits the phrase into words based on whitespace
    ...
    const results = await this.errsoleLogs.findAll({
      where,
      order,
      limit
    });
   ...
   */
  async getConfig (key) {
    const configEntry = await this.errsoleConfig.findOne({ where: { key } });
