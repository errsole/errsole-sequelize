...
class ErrsoleSequelize {
  ...
  async defineModels () {
    ...
    this.errsoleConfigs = this.sequelize.define('errsoleConfig', {
      ...
      timestamps: false // disabling the createdAt and updatedAtconsole.log(updateCount)   columns while creating the models
    });
  ...
  async createUser (user) {
    ...
    try {
      const result = await this.errsoleUsers.create({ name: user.name, email: user.email, password: user.password, role: user.role });
