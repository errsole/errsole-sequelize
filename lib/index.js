...
class ErrsoleSequelize extends EventEmitter {
  ...
  /**
  * Deletes a configuration record from the database.
  *
  * @async
  * @function deleteConfig
  * @param {string} key - The key identifying the configuration to be deleted.
  ...
  */
  async deleteConfig (key) {
    const result = await this.errsoleConfig.destroy({ where: { key } });
    if (result === 0) {
